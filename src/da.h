...
#define da_with_capacity(da, _capacity) do {\
		size_t type_size = _get_type_alignment(da.data + 1, da.data);\
...
#define da_clone(da_src, da_dst) do {\
		size_t type_size = _get_type_alignment(da_src.data + 1, da_src.data);\
		...
		memcpy(da_dst.data, da_src.data, total_size);\
	} while (0)\
